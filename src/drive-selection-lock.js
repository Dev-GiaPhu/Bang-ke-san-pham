...
(function(){
  ...
  const pathVariants=(rootName,p)=>({full:norm(fullPath(rootName,p)),rel:norm((p||[]).join(' / '))});
  function assetsFor(files,folderId){return files.filter(f=>(f.ancestors||[]).some(a=>a.id===folderId)).map(f=>({id:f.id,name:f.name,mimeType:f.mimeType||'',modifiedTime:f.modifiedTime||'',version:f.version||'',size:f.size||'',webViewLink:f.webViewLink||`https://drive.google.com/open?id=${f.id}`,thumbnailLink:f.thumbnailLink||'',folderPath:f.folderPath.join(' / '),role:/final/i.test(f.name)?'final':/mockup/i.test(f.name)?'mockup':/source/i.test(f.name)?'source':'asset'}))}
  function previews(assets){const a=assets.filter(x=>/\.(png|jpe?g|webp|gif|bmp|tiff?|svg)$/i.test(x.name));a.sort((x,y)=>(x.role==='final'?-1:0)-(y.role==='final'?-1:0)||String(y.modifiedTime).localeCompare(String(x.modifiedTime)));return a.slice(0,2)}
      ...
      for(const rootId of roots){
        ...
        for(const r of records.filter(x=>(x.driveRootFolderId||idFromUrl(x.driveRootFolderUrl||x.driveFolderUrl))===rootId)){
          ...
          if(desired){
            ...
            const matches=[...tree.folders.values()].filter(f=>{const p=pathVariants(tree.root,f.path);return p.full===want||p.rel===want});
            if(matches.length===1)chosen=matches[0];
          }
          if(!chosen&&r.driveVariantFolderId)chosen=tree.folders.get(r.driveVariantFolderId)||null;
          if(!chosen)continue;
          const assets=assetsFor(tree.files,chosen.id),pr=previews(assets),newPath=fullPath(tree.root,chosen.path);
          const next={driveRootFolderId:rootId,driveRootFolderUrl:r.driveRootFolderUrl||tree.root.webViewLink||url(rootId),driveVariantFolderId:chosen.id,driveFolderUrl:url(chosen.id),sourceFolderPath:newPath,drivePresent:true,assets,previewAssets:pr,previewFileId:pr[0]?.id||'',previewFileName:pr[0]?.name||'',lastDriveSync:new Date().toISOString()};
          if(r.driveVariantFolderId!==next.driveVariantFolderId||r.sourceFolderPath!==next.sourceFolderPath||JSON.stringify(r.assets||[])!==JSON.stringify(next.assets)){Object.assign(r,next);changed=true}
        }
      }
      if(changed){window.__gpManualDriveWrite=true;localStorage.setItem(STORE,JSON.stringify(records));window.__gpManualDriveWrite=false;window.dispatchEvent(new Event('gp-drive-selection-locked'))}
    }finally{running=false}
  }
  document.addEventListener('click',e=>{const b=e.target.closest?.('#confirm');if(b){window.__gpManualDriveWrite=true;setTimeout(()=>{window.__gpManualDriveWrite=false},2000)}},true);
  const nativeSet=Storage.prototype.setItem;
  Storage.prototype.setItem=function(k,v){
    if(k!==STORE||window.__gpManualDriveWrite)return nativeSet.call(this,k,v);
    try{
      const old=read(),next=JSON.parse(v),oldById=new Map(old.map(r=>[r.id,r]));
      const merged=next.map(r=>{const prev=oldById.get(r.id);if(!prev?.driveVariantFolderId||!r.driveVariantFolderId)return r;if(prev.driveVariantFolderId===r.driveVariantFolderId)return r;return {...r,driveRootFolderId:prev.driveRootFolderId||r.driveRootFolderId,driveRootFolderUrl:prev.driveRootFolderUrl||r.driveRootFolderUrl,driveVariantFolderId:prev.driveVariantFolderId,driveFolderUrl:prev.driveFolderUrl,sourceFolderPath:prev.sourceFolderPath,assets:prev.assets,previewAssets:prev.previewAssets,previewFileId:prev.previewFileId,previewFileName:prev.previewFileName,drivePresent:prev.drivePresent}});
