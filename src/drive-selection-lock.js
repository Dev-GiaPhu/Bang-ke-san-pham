...
(function(){
  ...
  const fullPath=(root,p)=>[root,...(p||[])].filter(Boolean).join(' / ');
  ...
  async function tree(token,rootId){
    const root=await api(token,`https://www.googleapis.com/drive/v3/files/${rootId}?fields=id,name,mimeType,webViewLink`),folders=new Map(),files=[];
    async function walk(id,p){
      folders.set(id,{id,name:p[p.length-1]||root.name,path:[...p]});
      let page='';do{const q=encodeURIComponent(`'${id}' in parents and trashed = false`),fields=encodeURIComponent('nextPageToken,files(id,name,mimeType,modifiedTime,version,size,webViewLink,thumbnailLink)');let u=`https://www.googleapis.com/drive/v3/files?q=${q}&pageSize=1000&fields=${fields}`;if(page)u+=`&pageToken=${encodeURIComponent(page)}`;const d=await api(token,u);for(const f of d.files||[]){if(f.mimeType==='application/vnd.google-apps.folder')await walk(f.id,[...p,f.name]);else if(ASSET.test(f.name))files.push({...f,folderPath:[...p]})}page=d.nextPageToken||''}while(page)
    }
    await walk(rootId,[]);return{root,folders,files};
  }
  function exactFolder(t,r){const desired=norm(r.sourceFolderPath||'');if(desired){const hits=[...t.folders.values()].filter(f=>norm(fullPath(t.root.name,f.path))===desired);if(hits.length===1)return hits[0]}return r.driveVariantFolderId?t.folders.get(r.driveVariantFolderId)||null:null}
  ...
  window.addEventListener('gp-drive-connected',()=>setTimeout(repairOnce,500));window.addEventListener('load',()=>setTimeout(repairOnce,1000));
})();