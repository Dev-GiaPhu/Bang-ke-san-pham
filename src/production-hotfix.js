...
const DRIVE_SESSION='gp-drive-token-v2';
    ...
    function wrapped(config){
      ...
      cfg.callback=(response)=>{
        if(response?.access_token && /drive\.readonly/.test(String(cfg.scope||''))){
          try{sessionStorage.setItem(DRIVE_SESSION,response.access_token)}catch{}
        }
...
async function openEditor(id,fromShare=false){
  let record=null;
  if(fromShare){const share=(location.hash.match(/^#share=([A-Za-z0-9_-]+)$/)||[])[1];if(!share)return;const snap=await getDoc(doc(db,'workspaces',WORKSPACE,'shares',share,'products',id));if(!snap.exists())return;record=snap.data()}
  else{const snap=await getDoc(doc(db,'workspaces',WORKSPACE,'products',id));if(!snap.exists())return;record=snap.data()}
  const types=Array.from(new Set([...(JSON.parse(localStorage.getItem('ventek-design-settings-v2')||'{}').editTypes||['Design mới','Sửa thông tin','Update thông tin','Sửa kích thước']),'Khác']));
...
function patchSettings(){const c=document.querySelector('#gp-client');if(!c)return;const root=c.closest('.actions');const email=auth.currentUser?.email||'';if(root && !email){return}const roleText=document.querySelector('.connection span')?.textContent||'';if(root&&roleText!=='Chủ sở hữu'){c.remove();root.querySelector('#gp-save-client')?.remove()}}
...
function patchShareDeletes(){document.querySelectorAll('[data-share-delete]').forEach(b=>{if(b.dataset.gpHotfix==='1')return;b.dataset.gpHotfix='1';b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();const row=b.closest('[data-share-row]');hardDeleteShare(b.dataset.shareDelete,row)},true)})}
